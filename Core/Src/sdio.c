...
void Get_CardInfo(void)
{
	...
	HAL_SD_GetCardCID(&hsd,&SDCardCID);
	HAL_SD_GetCardCSD(&hsd,&SDCardCSD);
	
	HAL_SD_GetCardInfo(&hsd,&SDCardInfo);
	
	//SDCardInfo.CardType
	log_info("SDCard CLK:%fMHz\r\n",(0.5*HAL_RCC_GetPCLK2Freq()/1000000)/(sdioclkdiv+2));
